  ...
  in hix ({config, build, lib, util, ...}: {
    ...
    compilers = {
      ...
      mwb.source.build = {
        ...
        hash = "sha256-41ngesQck2MlM4NvAtjAdNbF9GOu7zhZJwx0EDeABBY=";
    ...
    commands.comparison-1 = {
      expose = true;
      env = "buck";
      command = "${import ./ops/buck-test/comparison1 { inherit util; }}";
    };

    commands.comparison-2 = {
      expose = true;
      env = "buck";
      command = "${import ./ops/buck-test/comparison2 { inherit util; }}";
    };

    commands.three-layers = {
      expose = true;
      env = "buck";
      command = "${import ./ops/buck-test/three-layers { inherit util; }}";
    };
