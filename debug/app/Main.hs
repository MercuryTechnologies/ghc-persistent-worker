...
import Data.Functor ((<&>))
import Data.List (isPrefixOf, sortOn, uncons, sortBy)
  ...
  traverse_ \case
    stacks@((_, (_, Just SourceInformation {infoType}) : _) :| _)
      ...
      -> do
        ...
        displayStack (maybe id take limit (filter nonNullStack (fmap sanitizeStack <$> toList stacks)))
    stacks ->
...
strings =
  ...
  where
    ...
    target :: String
    target = "/nix/store/yzb1x2j00aak5i4p0758g581r8kdm2c5-dep27-1/lib/ghc-9.10.1/lib/x86_64-linux-ghc-9.10.1-inplace/libHSdep27-1-Lar0zi4gtbbQMjZHqIaDR-ghc9.10.1.so"
...
main :: IO ()
main = snapshotRun "buck-400" debug
