...
import GHC (DynFlags (..), Ghc, GhcMode (..), IsBootInterface (..), Logger, ModuleGraph, ModuleName (..), mkModuleGraph, getSessionDynFlags)
...
import Internal.Log (Log, setLogTarget)
...
loadCachedModule hsc_env unit (JsonFs name) CachedModule {sources, modules} = do
  ...
  summary <-
    -- TODO use the data from the interface json, needs to be loaded from Buck for package deps
    -- setHiLocation hsc_env <$>
...
loadCachedBuildPlan _logVar hsc_env0 dflags0 unit file = do
  ...
  where
    load worker_cache args_file = do
      hsc_env2 <- liftIO do
        ...
        -- TODO store one arg per line in the file instead of using @words@ and @unwords@
        (dflags1', _, _, _) <- parseFlags dflags0 hsc_env0.hsc_logger (buckLocation <$> words args)
...
computeMetadata env = do
  for_ env.args.unitArgs \ file -> writeFile file (unwords env.args.ghcOptions)
