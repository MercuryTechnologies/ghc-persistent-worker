...
import GHC.Driver.Env.Types (LinkDeps (..))

#else

import GHC.Linker.Deps (LinkDeps (..))
...
getLinkDeps
  ...
  -> LoaderState
  -- | THIS IS THE NEW API!
  --   Callback function to retrieve bytecode linkable.
  -> (HomeModInfo -> IO (Maybe Linkable))
  -> SrcSpan      -- for error messages
  -> [Module]     -- If you need these
  -> IO LinkDeps  -- ... then link these first
getLinkDeps opts interp pls lazyByteCode span mods = do
      ...
      withTiming (ldLogger opts) (text "getLinkDeps" <+> brackets (ppr span)) (const ()) $
        get_link_deps opts pls lazyByteCode maybe_normal_osuf span mods
...
get_link_deps
  ...
  -> [Module]
  -> IO LinkDeps
get_link_deps opts pls lazyByteCode maybe_normal_osuf span mods = do
  ...
  -- results.
    module_linkable = \case
      LinkHomeModule hmi ->
        -- NOTE: NEW MECHANISM
        -- invoke the lazyByteCode Callback function to retrieve bytecode linkable.
        adjust_linkable =<< maybe (expectJust "no cached bytecode" <$> lazyByteCode hmi) pure (homeModLinkable hmi)
          ...
          buildTwiceMsg = case (ghciWay, compWay) of
            ...
            _ ->
              text "  (2) Build the program twice: once" <+>
                pprWay' ghciWay Outputable.<> text ", and then" $$
              text "      " Outputable.<> pprWay' compWay <+>
