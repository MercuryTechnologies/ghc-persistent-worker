...
initialBuildUnit unit =
  unit {
    modules = [BuildModule {key, deps} | GenModule {key, deps} <- unit.modules]
  }
...
resumeBuildUnit unit =
  unit {
    modules = [BuildModule {key, deps = deps <> fold resumeDeps} | GenModule {key, deps, resumeDeps} <- unit.modules]
  }
